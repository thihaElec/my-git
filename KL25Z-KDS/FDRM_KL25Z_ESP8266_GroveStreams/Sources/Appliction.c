...
static uint8_t SendWebPage(uint8_t ch_id, bool ledIsOn, uint8_t temperature, const CLS1_StdIOType *io) {
  ...
  UTIL1_strcat(http, sizeof(http), (uint8_t*)"<h2>Web Server using KL25Z/ESP8266 (Thiha Electronics)</h2>\r\n");
  ...
  if (ESP_IsServerOn()) {
    ...
    if (res==ERR_OK) {
      if (isGet) { /* GET: put web page */
        res = SendWebPage(ch_id, LEDR_Get()!=FALSE, temp /*dummy temperature*/, io);
...
void APP_Run(void) {
  ...
  DHT_read(); // first data always have error
  WAIT1_Waitms(2000);

  while (ESP_IsServerOn()==TRUE);
...
//    temp = DHT_read();
    ...
    DHT_read();
    temp = (uint8_t)DHT_read_temp();
    humidity = (uint8_t)DHT_read_humidity();


    //    CLS1_SendStr("// 3.1 : ", io->stdOut);
    GROVESTREAMS_SendValue(temp,humidity, io);
//	CLS1_SendNum16u(temp, io->stdOut);
	LEDG_Off();
    WAIT1_Waitms(10000);

/*    WAIT1_Waitms(60000);  //5 mins
    WAIT1_Waitms(60000);
    WAIT1_Waitms(60000);
    WAIT1_Waitms(60000);
    WAIT1_Waitms(60000);
